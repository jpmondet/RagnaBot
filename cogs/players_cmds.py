...
class Players(commands.Cog):
    ...
    async def register(self, ctx, player_name: str = ""):
        for message in paginate(cml.registering_player(ctx.author.id, ctx.author.name, player_name)):
            msg_to_send = ''.join(message)
            await ctx.send(msg_to_send)
...
You can use !searchmap to find exactly what you need to copy/paste for !submit")
    ...
    async def submit_by_map_name(
        ...
        for message in paginate(cml.handle_player_submission(ctx.author.id, ctx.author.name,proof,difficulty,score, misses,perfects_percent,triggers, map_name,band,mapper)):
            msg_to_send = ''.join(message)
            await ctx.send(msg_to_send)
    ...
    async def submit(
        ...
        for message in paginate(cml.handle_player_submission(ctx.author.id, ctx.author.name,proof,difficulty,score, misses,perfects_percent,triggers,"","","", uuid)):
            msg_to_send = ''.join(message)
            await ctx.send(msg_to_send)
