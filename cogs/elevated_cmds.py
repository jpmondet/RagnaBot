...
class Moderators(commands.Cog):
    ...
    @commands.command(
        name="valid",
        help="Validate a pending submission with the number of the pending score (the number seen on `!listpending`). Exple : `!valid 1`",
    )
    @commands.has_any_role(*ROLE_ADMIN)
    @commands.before_invoke(record_usage)
    ...
    @commands.command(
        name="deny",
        help='Deny a pending submission with the number of the pending score (the number seen on `!listpending`). `!deny pending_number reason`, exple : `!deny 1 "Incorrect map name"`',
    )
    @commands.has_any_role(*ROLE_ADMIN)
    @commands.before_invoke(record_usage)
    async def deny(self, ctx, id_pending: int = -1, reason: str = ""):
        ...
        if id_pending <= 0:
            await ctx.send(
                "Please, indicate the id of the pending score to validate (shown with `!listpending`). For example : `!deny 23`"
            )
            return
    ...
    @commands.command(
        name="force-update-maps", help="Polls the Ragnasong.com API to update maps list"
    )
    @commands.has_any_role(*ROLE_ADMIN)
    @commands.before_invoke(record_usage)
