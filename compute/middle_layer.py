        ...
        top_players_list.append(
            {
                ...
                "total_score": f"{account['total_score']:.2f}",
                "total_misses": f"{str(account['total_misses'])}",
                "perfects_percent_avg": f"{str(perfects_percent_avg)}%",
                "total_triggers": f"{str(account['total_triggers'])}",
                "nb_songs_played": f"{str(nb_songs_played)}",
            }
        )
...
def get_top_players_on_specific_map(
    map_name: str, nb_to_show: int, format=True
) -> Union[str, List]:
    ...
    song_name = (
        f"{matching_song['artist']} - {matching_song['title']} - {matching_song['ownerUsername']}"
    )

    for difficulty in matching_song["difficulty"].split(","):
        map_scores_unsorted = list(
            dbl.get_score_by_map_uuid_and_diff(matching_song["uuid"], difficulty)
        )
        ...
        for rank, map_score in enumerate(map_scores):
            ...
            leaders_dict[f"{song_name} - Level {difficulty}"].append(
                {
                    "rank": f"{str(rank+1)}",
                    "player_name": f"{account['player_name']}",
                    "score": f"{map_score['score']}",
                    "misses": f"{str(map_score['misses'])}",
                    "perfects_percent": f"{str(map_score['perfects_percent'])}%",
                    "triggers": f"{str(map_score['triggers'])}",
                }
            )
            nb_to_show_temp -= 1
    leaders += "\n"
...
def unregistering_player(discord_id: int, discord_name: str, confirm: str) -> str:
    ...
    if confirm != "YEAHIMSURE!":
        return (
            f"{discord_name} didn't confirm, cancelling (send `!unregister YEAHIMSURE!` to confirm)"
        )
...
def get_pending_subs_player(
    discord_id: int = 0, format=True
) -> Union[str, Dict[int, Dict[str, Any]]]:
    ...
    for id_req, pdetails in enumerate(pendings):
        ...
        if format:
            pdetails_str: str = (
                f"        In-game name: {account['player_name']}  (Discord user : {account['discord_name']})"
                + "\n"
            )
...
def check_ldscore_parameters_submitted(
    proof: str, difficulty: str, score: str, misses: str, perfects_percent: str, triggers: str
):
...
def check_submission_by_name(
    ...
    if not attributes_presence(locals().values()):
        raise AttributeError(
            "Please fill all the field so admins can verify your submission easily :-). \n \
...
def check_submission(
    ...
    try:
        if uuid == "map":
            check_submission_by_name(
                proof, map_name, band, mapper, difficulty, score, misses, perfects_percent, triggers
            )
            map_submitted = list(dbl.search_map_by_title_artist_mapper(map_name, band, mapper))
            if not map_submitted:
                raise AttributeError(
                    "Sorry, not maps were found with those artist/title/mapper. Please try to search your map with `!searchmap 'anything'` before submitting"
                )
            if len(map_submitted) > 1:
                raise AttributeError(
                    "Hmm, looks like there are multiple maps with those artist/title/mapper.  \n \
                    Please try to search your map with `!searchmap 'anything'` to submit with uuid instead"
                )
            map_submitted = map_submitted[0]
        else:
            check_submission_by_uuid(
                proof, uuid, difficulty, score, misses, perfects_percent, triggers
            )
            map_submitted = dbl.get_map_by_uuid(uuid)
            if not map_submitted:
                raise AttributeError(
                    "Sorry, not maps were found with this uuid. Please try to search your map with `!searchmap 'anything'`"
                )

        parameter_error = check_ldscore_parameters_submitted(
            proof, difficulty, score, misses, perfects_percent, triggers
        )
        if parameter_error:
        ...
        if str(difficulty) not in map_submitted["difficulty"]:
            raise AttributeError(
                f"Sorry, the map's difficulty you submitted doesn't exist. This map only have those : {map_submitted['difficulty']}"
            )
...
def handle_player_submission(
    ...
    try:
        map_submitted = check_submission(
            uuid,
            proof,
            map_name,
            band,
...
def validate_submission(id_pending: int = -1, api_call=False):
    ...
    old_score = dbl.get_score_by_player_id_map_uuid_diff(
        valid_score["player_id"], valid_score["map_uuid"], valid_score["difficulty_played"]
    )
    if old_score:
        print(old_score)
