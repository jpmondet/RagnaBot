...
def unregister(discord_user: DiscordUser, confirm_string: str, credentials = Depends(check_credentials)):
    return cml.unregistering_player(discord_user.discord_user_id, discord_user.discord_user_name, confirm_string)
...
def submit(score: Score, credentials = Depends(check_credentials)):
    return cml.handle_player_submission(score.discord_user_id, score.discord_user_name, score.proof, score.difficulty, score.score, score.misses,
            score.perfects_percent, score.triggers,"","","", score.map_uuid)
